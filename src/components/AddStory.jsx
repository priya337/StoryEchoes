...
const movePage = (pageIndex, direction) => {
  ...
  <PollinationImage prompt={text} onComplete={(url)=> {
    ...
    if (pages.length < MAX_PAGES) { setPages((prevPages)=> [
      ...
      mediaUrlError: "Invalid audio URL. Supported formats: .mp3, .mp4, .wav, .ogg.",
      }
      ...
      i === index ? { ...page, mediaUrl: url, mediaUrlError: null } : page
        ...
        useEffect(() => {
          if (generalErrorMessage) {
            const errorMessageContainer = document.querySelector(".general-error-message");
        ...
        const handleSubmit = async () => {
          ...
          try {
            ...
            if (response.ok) {
              ...
              setBeeSubmitMessage("🐝 Hooray! Your story has been submitted successfully!");
      ...
      return (
      <div className="add-story-container" style={{ fontFamily: "Comic Neuve, cursive" }}>
        <h1 style={{ fontFamily: "Comic Neuve, cursive", fontWeight: "bold", fontSize: "2rem" }}>Create Your Magical
          Adventure</h1>
        <form className="story-form">
          ...
          <div className="row">
            <div className={`form-group ${errors.title ? "error-highlight" : "" }`}>
              ...
              {errors.title && <span className="error"
                style={{ fontFamily: "Comic Neuve, cursive" }}>{errors.title}</span>}
            </div>
            ...
            <div className={`form-group ${errors.author ? "error-highlight" : "" }`}>
              ...
              style={{ fontFamily: "Comic Neuve, cursive" }}
              />
              {errors.author && <span className="error"
                style={{ fontFamily: "Comic Neuve, cursive" }}>{errors.author}</span>}
            </div>
          </div>

          {/* Front Cover */}
          <div className={`form-group front-cover-group ${errors.frontCover ? "error-highlight" : "" }`}>
            <label style={{ fontFamily: "Comic Neuve, cursive", fontWeight: "bold", fontSize: "1.5rem" , marginLeft: "10px"}}>Front
              Cover</label>
            <input type="file" accept="image/*" onChange={(e)=> handleFileUpload(e, null, "image")} style={{ width: "300px" }}
            />
            {frontCover && (
          ...
          {/* Story Content Label */}
          <h2
            style={{ color: "#28c4ac", textAlign: "center", margin: "20px 0", fontWeight: "bold", fontSize: "1.5rem" }}>
            Story Content</h2>

          {/* Pages Section */}
          <div ref={pagesContainerRef} className="pages-container" style={{ overflowY: "scroll", overflowX: "hidden", maxHeight: "400px" }}>
            {pages.map((page, index) => (
            <div key={index} className="page-input-group">
                  {/* Page Number Below the Thick Line */}
  
                  {page.page > 1 ? (
        <h3 style={{ fontFamily: "Bubblegum Sans", marginTop: "0" , marginBottom: "-70px"  }}>Page {page.page}</h3>
      ) : (
      ...
      {page.page > 1 && (
        <div style={{ width: "800%", height: "30px", backgroundColor: "darkblue", marginBottom: "30px", marginLeft: "-15px" }}></div>
      )}
  ...
  <div key={index} className={`page ${errors.pages?.includes(`Page ${page.page}`) ? "error-highlight" : ""}`}>
              ...
              <div onClick={()=> alert(`Transcription clicked for Page ${page.page}`)}
                style={{ cursor: "pointer", fontSize: "1rem", textAlign: "center" }}
                >
                {page.beeMessage}
                <br />
              ...
              <textarea value={page.text} placeholder="Write your story here..." onChange={(e)=> handlePageTextChange(e.target.value, index)}
                style={{ fontFamily: "Bubblegum Sans, cursive", height: "150px", marginBottom: "10px"}}
              />
